...
public class InMemoryRegistryStorage implements RegistryStorage {
    ...
    private final Map<String, RegistryStorage> tenantStorages = new ConcurrentHashMap<>();
    ...
    private RegistryStorage tenantStorage() {
        return tenantStorages.computeIfAbsent(tenantContext.tenantId(), (k) -> {
                SimpleInMemoryRegistryStorage storage = new SimpleInMemoryRegistryStorage();
                storage.setArtifactTypeProviderFactory(factory);
                storage.setSecurityIdentity(securityIdentity);
                storage.init();
                return storage;
            });
    }
